...
using MainProject.Controllers.CustomMunicipios.Controllers;
...
using MainProject.Controllers.CustomMunicipios.Models.MunicipiosAll.ObtainMunicipiosAllObjectListAzync.FnRes;
using MainProject.Controllers.CustomMunicipios.Models.MunicipiosAll.RetrieveDbMunicipiosAllAzync;
...
using MainProject.Controllers.CustomMunicipios.Models.MunicipiosFiltered.ObtainMunicipiosObjectListAzync;
using MainProject.Controllers.CustomMunicipios.Models.MunicipiosFiltered.MunicipiosDefaultFilterParams;
using MainProject.Services.CustomHelper.Models.FnRes;
using MainProject.Controllers.CustomMunicipios.Models.MunicipiosFiltered.ObtainTotalRowsFilteredWithOutLimitMunicipios;
using MainProject.Services.CustomSybase.Models.QueryAAAModelDefaultFnResFTRowsAzync;

namespace MainProject.Controllers.CustomMunicipios.Services.Impl
{
    public class CustomMunicipiosService : ICustomMunicipios
    {
        // INJECTED
        ...
        // ========== CONSTRUCTOR ========= //
        public CustomMunicipiosService(
        ...
        )
        {
            ...
            // ===== TRANSLATION INIT =====
            var controllerType = typeof(CustomMunicipiosController);
            ...
            _localizer = factory.Create("CustomMunicipios.Resources.CustomMunicipiosController", assemblyObj.Name);
            // ===== TRANSLATION END =====
        ...
        public async Task<string> MainGetMunicipiosFilteredAsync(string ls_ctrl_json)
        {
            FnResMainGetMunicipiosFilteredDev objFnResDev = new();
            ...
            if (true)
            {
                ...
                if (!httpContext.Request.Headers.TryGetValue("Authorization", out var authorizationHeader))
                {
                    ...
                    return this.ResponseFormatGetMunicipiosFiltered(objFnResDev, lb_developer_mode);
                }
            ...
            if (true)
            {
                ...
                List<ModelMunicipioJsonCustom> rowList = new(); // Retornaré customizado por defecto
                ...
                FnResObtainMunicipiosObjList ObtainFilteredObjListResult = await this.ObtainMunicipiosObjectListAzync();
                ...
                if (ObtainFilteredObjListResult.Ksuccess && ObtainFilteredObjListResult.Kdata.Count >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        {
                            objFnResDev.Kdata.Add(new ModelMunicipioJsonCustom());
                            var currentModel = ObtainFilteredObjListResult.Kdata[i];

                            objFnResDev.Kdata[i].Id = currentModel.Id;
                            objFnResDev.Kdata[i].Id_ciudad = currentModel.Id_ciudad?.Trim();
                            objFnResDev.Kdata[i].Id_departamento = currentModel.Id_departamento?.Trim();
                            objFnResDev.Kdata[i].Nombre_ciudad = currentModel.Nombre_ciudad?.Trim();
                ...
                var ObtainTotalRowsFilWithOutLimitCliRs = await this.ObtainTotalRowsFilteredWithOutLimitMunicipios();
                // ERROR ???
        ...
        public async Task<FnResObtainMunicipiosObjList> ObtainMunicipiosObjectListAzync()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainMunicipiosObjList objFnResDev = new();
            ...
            try
            {
                ls_jsonresponse_retrieve_filtered = await this.RetrieveDbAllMunicipiosFilteredAzync(); // get rows filtered and limited                
            }
            ...
            ResAjaxMunicipiosFilteredAzync dbSearchResult = new(); // Force to be an object
            ResAjaxMunicipiosFilteredAzync? dbSearchResultOrNull = null; // necesary variable to try catch to cast postman json object            
            try
            {
                dbSearchResultOrNull = JsonSerializer.Deserialize<ResAjaxMunicipiosFilteredAzync>(ls_jsonresponse_retrieve_filtered, _currentJsonOptions);
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            dbSearchResult = dbSearchResultOrNull ?? new ResAjaxMunicipiosFilteredAzync();
        ...
        // Define a custom helper
        public async Task<string> RetrieveDbAllMunicipiosFilteredAzync()
        {
            FnResMunicipiosDefaultFilterParams aParams = this.MunicipiosDefaultFilterParams();
            ...
            string ls_jsonres = await _customSybaseService.QueryMunicipiosFilteredAzync(la_params_remote); // Esperamos una json de respuesta            
            return ls_jsonres;
        }


        // Define a custom helper
        public FnResMunicipiosDefaultFilterParams MunicipiosDefaultFilterParams()
        {
            FnResMunicipiosDefaultFilterParams aParams = new();
        ...
        // Define a custom helper
        public async Task<FnResObtainTotalRowsFWOLMunicipios> ObtainTotalRowsFilteredWithOutLimitMunicipios()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainTotalRowsFWOLMunicipios objFnResDev = new();
            ...
            try
            {
                ls_database_response = await this.RetrieveDbAllMunicipiosFilteredTotalRowsAzync();
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
        ...
        //// Define a custom helper
        public async Task<string> RetrieveDbAllMunicipiosFilteredTotalRowsAzync()
        {
            FnResMunicipiosDefaultFilterParams aParams = this.MunicipiosDefaultFilterParams();
            ...
            string ls_jsonres = await _customSybaseService.QueryMunicipiosFilteredTotalRowsAzync(la_params_remote); // Esperamos una json de respuesta
            return ls_jsonres;
        }
        ...
        private string ResponseFormatGetMunicipiosFiltered(FnResMainGetMunicipiosFilteredDev stdFnResDev, bool developerMode = false)
        {
            var httpContext = _customHelper.GetCurrentHttpContext();
            var Request = httpContext.Request;
        ...
        // API publica por ahora
        public async Task<string> MainGetMunicipiosAllAsync(string ls_ctrl_json)
        {
            FnResMainGetMunicipiosAllDev objFnResDev = new();
            ...
            if (true)
            {
                ...
                if (!httpContext.Request.Headers.TryGetValue("Authorization", out var authorizationHeader))
                {
                    ...
                    return ResponseFormatGetMunicipiosAll(objFnResDev, lb_developer_mode);
                }
            ...
            if (true)
            {
                // Llamar a obtain
                // BD CONSULTAR REGISTROS FILTRADOS
                FnResObtainMunicipiosAllObjectList obtainModelAllObjListResult = await this.ObtainMunicipiosAllObjectListAzync();
                ...
                if (obtainModelAllObjListResult.Ksuccess && obtainModelAllObjListResult.Kdata.Count >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        for (int i = 0; i < li_total_registros; i++)
                        {
                            objFnResDev.Kdata.Add(new ModelMunicipioAllJsonCustom());
            ...
            // Return expected response
            return ResponseFormatGetMunicipiosAll(objFnResDev, lb_developer_mode); // on fail json received, activate developerMode
        }


        // Define a custom helper
        private string ResponseFormatGetMunicipiosAll(FnResMainGetMunicipiosAllDev stdFnResDev, bool developerMode = false)
        {
        ...
        // Define a custom helper
        public async Task<FnResObtainMunicipiosAllObjectList> ObtainMunicipiosAllObjectListAzync()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainMunicipiosAllObjectList objFnResDev = new();
            ...
            try
            {
                ls_jsonresponse_bdtable_all = await this.RetrieveDbMunicipiosAllAzync(); // get all rows                
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            // ERRORRR PARSING JSON???
            ResAjaxMunicipiosAllAzync dbSearchResult = new(); // Force to be an object
            ResAjaxMunicipiosAllAzync? dbSearchResultOrNull = null; // necesary variable to try catch to cast postman json object            
            try
            {
                dbSearchResultOrNull = JsonSerializer.Deserialize<ResAjaxMunicipiosAllAzync>(ls_jsonresponse_bdtable_all, _currentJsonOptions);
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            dbSearchResult = dbSearchResultOrNull ?? new ResAjaxMunicipiosAllAzync();
        ...
        public async Task<string> RetrieveDbMunicipiosAllAzync()
        {
            string ls_jsonres = await _customSybaseService.QueryMunicipiosAllAzync();
            return ls_jsonres;
        }
