...
using MainProject.Controllers.CmUsuariosMovil.Controllers;
...
using MainProject.Controllers.CmUsuariosMovil.Models.CmUsuariosMovilAll.ObtainCmUsuariosMovilAllObjectListAzync.FnRes;
using MainProject.Controllers.CmUsuariosMovil.Models.CmUsuariosMovilAll.RetrieveDbCmUsuariosMovilAllAzync;
...
using MainProject.Controllers.CmUsuariosMovil.Models.CmUsuariosMovilFiltered.ObtainCmUsuariosMovilObjectListAzync;
using MainProject.Controllers.CmUsuariosMovil.Models.CmUsuariosMovilFiltered.CmUsuariosMovilDefaultFilterParams;
using MainProject.Services.CustomHelper.Models.FnRes;
using MainProject.Controllers.CmUsuariosMovil.Models.CmUsuariosMovilFiltered.ObtainTotalRowsFilteredWithOutLimitCmUsuariosMovil;
using MainProject.Services.CustomSybase.Models.QueryAAAModelDefaultFnResFTRowsAzync;

namespace MainProject.Controllers.CmUsuariosMovil.Services.Impl
{
    public class CmUsuariosMovilService : ICmUsuariosMovil
    {
        // INJECTED
        ...
        )
        {
            ...
            // ===== TRANSLATION INIT =====
            var controllerType = typeof(CmUsuariosMovilController);
            ...
            _localizer = factory.Create("CmUsuariosMovil.Resources.CmUsuariosMovilController", assemblyObj.Name);
            // ===== TRANSLATION END =====
        ...
        public async Task<string> MainGetCmUsuariosMovilFilteredAsync(string ls_ctrl_json)
        {
            FnResMainGetCmUsuariosMovilFilteredDev objFnResDev = new();
            ...
            if (true)
            {
                ...
                if (!httpContext.Request.Headers.TryGetValue("Authorization", out var authorizationHeader))
                {
                    ...
                    return this.ResponseFormatGetCmUsuariosMovilFiltered(objFnResDev, lb_developer_mode);
                }
            ...
            if (true)
            {
                ...
                List<ModelCmUsuariosMovilJsonCustom> rowList = new(); // Retornaré customizado por defecto
                ...
                FnResObtainCmUsuariosMovilObjList ObtainFilteredObjListResult = await this.ObtainCmUsuariosMovilObjectListAzync();
                ...
                if (ObtainFilteredObjListResult.Ksuccess && ObtainFilteredObjListResult.Kdata.Count >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        var user = new ModelCmUsuariosMovilJsonCustom();
                        
                        for (int i = 0; i < li_total_registros; i++)
                        {
                            objFnResDev.Kdata.Add(new ModelCmUsuariosMovilJsonCustom());
                            var currentModel = ObtainFilteredObjListResult.Kdata[i];

                            objFnResDev.Kdata[i].Id = currentModel.Id;
                            objFnResDev.Kdata[i].Codigo_usuario = currentModel.Codigo_usuario?.Trim();
                            objFnResDev.Kdata[i].Nombre_completo = currentModel.Nombre_completo?.Trim();
                            objFnResDev.Kdata[i].Zona = currentModel.Zona?.Trim();
                            objFnResDev.Kdata[i].Password = currentModel.Password?.Trim();
                            objFnResDev.Kdata[i].Nit = currentModel.Nit?.Trim();
                ...
                var ObtainTotalRowsFilWithOutLimitCliRs = await this.ObtainTotalRowsFilteredWithOutLimitCmUsuariosMovil();
                // ERROR ???
        ...
        // API publica por ahora
        public async Task<string> MainGetCmUsuariosMovilAllAsync(string ls_ctrl_json)
        {
            FnResMainGetCmUsuariosMovilAllDev objFnResDev = new();
            ...
            if (true)
            {
                ...
                if (!httpContext.Request.Headers.TryGetValue("Authorization", out var authorizationHeader))
                {
                    ...
                    return ResponseFormatGetCmUsuariosMovilAll(objFnResDev, lb_developer_mode);
                }
            ...
            if (true)
            {
                // Llamar a obtain
                // BD CONSULTAR REGISTROS FILTRADOS
                FnResObtainCmUsuariosMovilAllObjectList obtainModelAllObjListResult = await this.ObtainCmUsuariosMovilAllObjectListAzync();
                ...
                if (obtainModelAllObjListResult.Ksuccess && obtainModelAllObjListResult.Kdata.Count >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        for (int i = 0; i < li_total_registros; i++)
                        {
                            objFnResDev.Kdata.Add(new ModelCmUsuariosMovilAllJsonCustom());
            ...
            // Return expected response
            return ResponseFormatGetCmUsuariosMovilAll(objFnResDev, lb_developer_mode); // on fail json received, activate developerMode
        }


        // Define a custom helper
        public async Task<FnResObtainCmUsuariosMovilObjList> ObtainCmUsuariosMovilObjectListAzync()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainCmUsuariosMovilObjList objFnResDev = new();
            ...
            try
            {
                ls_jsonresponse_retrieve_filtered = await this.RetrieveDbAllCmUsuariosMovilFilteredAzync(); // get rows filtered and limited                
            }
            ...
            ResAjaxCmUsuariosMovilFilteredAzync dbSearchResult = new(); // Force to be an object
            ResAjaxCmUsuariosMovilFilteredAzync? dbSearchResultOrNull = null; // necesary variable to try catch to cast postman json object            
            try
            {
                dbSearchResultOrNull = JsonSerializer.Deserialize<ResAjaxCmUsuariosMovilFilteredAzync>(ls_jsonresponse_retrieve_filtered, _currentJsonOptions);
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            dbSearchResult = dbSearchResultOrNull ?? new ResAjaxCmUsuariosMovilFilteredAzync();
        ...
        // Define a custom helper
        public async Task<string> RetrieveDbAllCmUsuariosMovilFilteredAzync()
        {
            FnResCmUsuariosMovilDefaultFilterParams aParams = this.CmUsuariosMovilDefaultFilterParams();
            ...
            string ls_jsonres = await _customSybaseService.QueryCmUsuariosMovilFilteredAzync(la_params_remote); // Esperamos una json de respuesta            
            return ls_jsonres;
        }


        // Define a custom helper
        public FnResCmUsuariosMovilDefaultFilterParams CmUsuariosMovilDefaultFilterParams()
        {
            FnResCmUsuariosMovilDefaultFilterParams aParams = new();
        ...
        // Define a custom helper
        public async Task<FnResObtainTotalRowsFWOLCmUsuariosMovil> ObtainTotalRowsFilteredWithOutLimitCmUsuariosMovil()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainTotalRowsFWOLCmUsuariosMovil objFnResDev = new();
            ...
            try
            {
                ls_database_response = await this.RetrieveDbAllCmUsuariosMovilFilteredTotalRowsAzync();
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
        ...
        //// Define a custom helper
        public async Task<string> RetrieveDbAllCmUsuariosMovilFilteredTotalRowsAzync()
        {
            FnResCmUsuariosMovilDefaultFilterParams aParams = this.CmUsuariosMovilDefaultFilterParams();
            ...
            string ls_jsonres = await _customSybaseService.QueryCmUsuariosMovilFilteredTotalRowsAzync(la_params_remote); // Esperamos una json de respuesta
            return ls_jsonres;
        }
        ...
        private string ResponseFormatGetCmUsuariosMovilFiltered(FnResMainGetCmUsuariosMovilFilteredDev stdFnResDev, bool developerMode = false)
        {
            var httpContext = _customHelper.GetCurrentHttpContext();
            var Request = httpContext.Request;
        ...
        // Define a custom helper
        private string ResponseFormatGetCmUsuariosMovilAll(FnResMainGetCmUsuariosMovilAllDev stdFnResDev, bool developerMode = false)
        {
            ...
            var objFnResPro = new
            {
                ...
                KerrorCode = stdFnResDev.KerrorCode,
                Kmessage = stdFnResDev.Kmessage,
                Kldt_fecha_servidor_ase = stdFnResDev.Kldt_fecha_servidor_ase,
                Kdata = stdFnResDev.Kdata
            };
            return JsonSerializer.Serialize(objFnResPro, _currentJsonOptions);
        }
        ...
        // Define a custom helper
        public async Task<FnResObtainCmUsuariosMovilAllObjectList> ObtainCmUsuariosMovilAllObjectListAzync()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainCmUsuariosMovilAllObjectList objFnResDev = new();
            ...
            try
            {
                ls_jsonresponse_bdtable_all = await this.RetrieveDbCmUsuariosMovilAllAzync(); // get all rows                
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            // ERRORRR PARSING JSON???
            ResAjaxCmUsuariosMovilAllAzync dbSearchResult = new(); // Force to be an object
            ResAjaxCmUsuariosMovilAllAzync? dbSearchResultOrNull = null; // necesary variable to try catch to cast postman json object            
            try
            {
                dbSearchResultOrNull = JsonSerializer.Deserialize<ResAjaxCmUsuariosMovilAllAzync>(ls_jsonresponse_bdtable_all, _currentJsonOptions);
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            dbSearchResult = dbSearchResultOrNull ?? new ResAjaxCmUsuariosMovilAllAzync();
        ...
        public async Task<string> RetrieveDbCmUsuariosMovilAllAzync()
        {
            string ls_jsonres = await _customSybaseService.QueryCmUsuariosMovilAllAzync();
            return ls_jsonres;
        }
