...
using MainProject.Controllers.CustomDepartamentos.Controllers;
...
using MainProject.Controllers.CustomDepartamentos.Models.DepartamentosAll.ObtainDepartamentosAllObjectListAzync.FnRes;
using MainProject.Controllers.CustomDepartamentos.Models.DepartamentosAll.RetrieveDbDepartamentosAllAzync;
...
using MainProject.Controllers.CustomDepartamentos.Models.DepartamentosFiltered.ObtainDepartamentosObjectListAzync;
using MainProject.Controllers.CustomDepartamentos.Models.DepartamentosFiltered.DepartamentosDefaultFilterParams;
using MainProject.Services.CustomHelper.Models.FnRes;
using MainProject.Controllers.CustomDepartamentos.Models.DepartamentosFiltered.ObtainTotalRowsFilteredWithOutLimitDepartamentos;
using MainProject.Services.CustomSybase.Models.QueryAAAModelDefaultFnResFTRowsAzync;

namespace MainProject.Controllers.CustomDepartamentos.Services.Impl
{
    public class CustomDepartamentosService : ICustomDepartamentos
    {
        // INJECTED
        ...
        // ========== CONSTRUCTOR ========= //
        public CustomDepartamentosService(
        ...
        )
        {
            ...
            // ===== TRANSLATION INIT =====
            var controllerType = typeof(CustomDepartamentosController);
            ...
            _localizer = factory.Create("CustomDepartamentos.Resources.CustomDepartamentosController", assemblyObj.Name);
            // ===== TRANSLATION END =====
        ...
        public async Task<string> MainGetDepartamentosFilteredAsync(string ls_ctrl_json)
        {
            FnResMainGetDepartamentosFilteredDev objFnResDev = new();
            ...
            if (true)
            {
                ...
                if (!httpContext.Request.Headers.TryGetValue("Authorization", out var authorizationHeader))
                {
                    ...
                    return this.ResponseFormatGetDepartamentosFiltered(objFnResDev, lb_developer_mode);
                }
            ...
            if (true)
            {
                ...
                List<ModelDepartamentoJsonCustom> rowList = new(); // Retornaré customizado por defecto
                ...
                FnResObtainDepartamentosObjList ObtainFilteredObjListResult = await this.ObtainDepartamentosObjectListAzync();
                ...
                if (ObtainFilteredObjListResult.Ksuccess && ObtainFilteredObjListResult.Kdata.Count >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        {
                            objFnResDev.Kdata.Add(new ModelDepartamentoJsonCustom());
                            var currentModel = ObtainFilteredObjListResult.Kdata[i];

                            objFnResDev.Kdata[i].Id = currentModel.Id;
                            objFnResDev.Kdata[i].Id_departamento = currentModel.Id_departamento?.Trim();
                ...
                var ObtainTotalRowsFilWithOutLimitCliRs = await this.ObtainTotalRowsFilteredWithOutLimitDepartamentos();
                // ERROR ???
        ...
        public async Task<FnResObtainDepartamentosObjList> ObtainDepartamentosObjectListAzync()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainDepartamentosObjList objFnResDev = new();
            ...
            try
            {
                ls_jsonresponse_retrieve_filtered = await this.RetrieveDbAllDepartamentosFilteredAzync(); // get rows filtered and limited                
            }
            ...
            ResAjaxDepartamentosFilteredAzync dbSearchResult = new(); // Force to be an object
            ResAjaxDepartamentosFilteredAzync? dbSearchResultOrNull = null; // necesary variable to try catch to cast postman json object            
            try
            {
                dbSearchResultOrNull = JsonSerializer.Deserialize<ResAjaxDepartamentosFilteredAzync>(ls_jsonresponse_retrieve_filtered, _currentJsonOptions);
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            dbSearchResult = dbSearchResultOrNull ?? new ResAjaxDepartamentosFilteredAzync();
        ...
        // Define a custom helper
        public async Task<string> RetrieveDbAllDepartamentosFilteredAzync()
        {
            FnResDepartamentosDefaultFilterParams aParams = this.DepartamentosDefaultFilterParams();
            ...
            string ls_jsonres = await _customSybaseService.QueryDepartamentosFilteredAzync(la_params_remote); // Esperamos una json de respuesta
            return ls_jsonres;
        }


        // Define a custom helper
        public FnResDepartamentosDefaultFilterParams DepartamentosDefaultFilterParams()
        {
            FnResDepartamentosDefaultFilterParams aParams = new();
        ...
        // Define a custom helper
        public async Task<FnResObtainTotalRowsFWOLDepartamentos> ObtainTotalRowsFilteredWithOutLimitDepartamentos()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainTotalRowsFWOLDepartamentos objFnResDev = new();
            ...
            try
            {
                ls_database_response = await this.RetrieveDbAllDepartamentosFilteredTotalRowsAzync();
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
        ...
        //// Define a custom helper
        public async Task<string> RetrieveDbAllDepartamentosFilteredTotalRowsAzync()
        {
            FnResDepartamentosDefaultFilterParams aParams = this.DepartamentosDefaultFilterParams();
            ...
            string ls_jsonres = await _customSybaseService.QueryDepartamentosFilteredTotalRowsAzync(la_params_remote); // Esperamos una json de respuesta
            return ls_jsonres;
        }
        ...
        private string ResponseFormatGetDepartamentosFiltered(FnResMainGetDepartamentosFilteredDev stdFnResDev, bool developerMode = false)
        {
            var httpContext = _customHelper.GetCurrentHttpContext();
            var Request = httpContext.Request;
        ...
        // API publica por ahora
        public async Task<string> MainGetDepartamentosAllAsync(string ls_ctrl_json)
        {
            FnResMainGetDepartamentosAllDev objFnResDev = new();
            ...
            if (true)
            {
                ...
                if (!httpContext.Request.Headers.TryGetValue("Authorization", out var authorizationHeader))
                {
                    ...
                    return ResponseFormatGetDepartamentosAll(objFnResDev, lb_developer_mode);
                }
            ...
            if (true)
            {
                // Llamar a obtain
                // BD CONSULTAR REGISTROS FILTRADOS
                FnResObtainDepartamentosAllObjectList obtainModelAllObjListResult = await this.ObtainDepartamentosAllObjectListAzync();
                ...
                if (obtainModelAllObjListResult.Ksuccess && obtainModelAllObjListResult.Kdata.Count >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        for (int i = 0; i < li_total_registros; i++)
                        {
                            objFnResDev.Kdata.Add(new ModelDepartamentoAllJsonCustom());
            ...
            // Return expected response
            return ResponseFormatGetDepartamentosAll(objFnResDev, lb_developer_mode); // on fail json received, activate developerMode
        }


        // Define a custom helper
        private string ResponseFormatGetDepartamentosAll(FnResMainGetDepartamentosAllDev stdFnResDev, bool developerMode = false)
        {
        ...
        // Define a custom helper
        public async Task<FnResObtainDepartamentosAllObjectList> ObtainDepartamentosAllObjectListAzync()
        {
            // INICIALIZAMOS VALORES DEFECTO DE RESPUESTA
            FnResObtainDepartamentosAllObjectList objFnResDev = new();
            ...
            try
            {
                ls_jsonresponse_bdtable_all = await this.RetrieveDbDepartamentosAllAzync(); // get all rows                
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            // ERRORRR PARSING JSON???
            ResAjaxDepartamentosAllAzync dbSearchResult = new(); // Force to be an object
            ResAjaxDepartamentosAllAzync? dbSearchResultOrNull = null; // necesary variable to try catch to cast postman json object            
            try
            {
                dbSearchResultOrNull = JsonSerializer.Deserialize<ResAjaxDepartamentosAllAzync>(ls_jsonresponse_bdtable_all, _currentJsonOptions);
            }
            catch (JsonException e)
            {
                // Header response [500] Server error
            ...
            dbSearchResult = dbSearchResultOrNull ?? new ResAjaxDepartamentosAllAzync();
        ...
        public async Task<string> RetrieveDbDepartamentosAllAzync()
        {
            string ls_jsonres = await _customSybaseService.QueryDepartamentosAllAzync();
            return ls_jsonres;
        }
